...
export function VaultCard({ item }: { item: ComponentEntry }) {
  ...
  return (
    <Link href={href}>
      <div
        className="group flex flex-col bg-white dark:bg-[#211e1e] hover:bg-[#f8f9fa] dark:hover:bg-[#2d2d2d] border border-black/5 dark:border-white/5 rounded-[10px] transition-all duration-300 shadow-sm cursor-pointer p-1 hover:-translate-y-1"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <div className="relative w-full aspect-[1.7] overflow-hidden bg-gray-100 dark:bg-muted/20 rounded-[10px] border border-black/5 dark:border-white/5">
          {item.isPremium && (
