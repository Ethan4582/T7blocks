...
export function VaultCard({ item }: { item: ComponentEntry }) {
  ...
  return (
    <div
      className="group relative flex flex-col bg-white dark:bg-[#211e1e] hover:bg-[#f8f9fa] dark:hover:bg-[#2d2d2d] border border-black/5 dark:border-white/5 rounded-[10px] transition-all duration-300 shadow-sm p-1 hover:-translate-y-1"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Primary Link Overlay */}
      <Link href={href} className="absolute inset-0 z-0" aria-label={`View ${item.displayName}`} />

      <div className="relative w-full aspect-[1.7] overflow-hidden bg-gray-100 dark:bg-muted/20 rounded-[10px] border border-black/5 dark:border-white/5 z-10 pointer-events-none">
        {item.isPremium && (
          <div className="absolute top-3 left-3 z-20">
            <div className="flex items-center gap-1.5 bg-white/80 dark:bg-background/80 border border-black/10 dark:border-border/30 backdrop-blur-md px-2.5 py-1 rounded-full shadow-sm">
        ...
        {!item.isPremium && (
          <div className={`absolute top-3 right-3 z-20 flex gap-2 transition-all duration-500 pointer-events-auto ${isHovered ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>
            <a
            ...
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                toggleBookmark(item.name, e);
              }}
              className={`w-8 h-8 rounded-full border border-black/10 dark:border-white/10 flex items-center justify-center shadow-xl transition-all ${bookmarked ? "bg-accent text-accent-foreground" : "bg-white/60 dark:bg-background/60 backdrop-blur-md text-foreground/70"}`}
            >
      ...
      <div className="px-1.5 pt-4 pb-1.5 flex justify-between items-center bg-transparent mt-1 relative z-10 pointer-events-none">
